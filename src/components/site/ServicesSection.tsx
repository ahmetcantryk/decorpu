    ...
    <section id="hizmetler" className="scroll-mt-20 border-t border-line bg-bg-subtle py-20">
      <Container>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="font-mono text-xs uppercase tracking-wide text-accent">Ne yapıyoruz</p>
            <h2 className="mt-2 text-3xl sm:text-4xl">Hizmetlerimiz</h2>
            <p className="mt-2 max-w-xl text-muted">Keşiften imalata, montajdan özel tasarıma — uçtan uca proje desteği.</p>
          </div>
            ...
            <li key={s.id}>
              <Link href={`/hizmetler/${s.slug}`} className="group flex h-full flex-col overflow-hidden rounded-lg border border-line bg-surface transition-shadow hover:shadow-soft">
                <div className="relative aspect-[16/10] overflow-hidden bg-bg-subtle">
                  {s.image_url ? (
                    <Image src={s.image_url} alt={s.title} fill sizes="(max-width: 1024px) 100vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.04]" />
                  ) : null}

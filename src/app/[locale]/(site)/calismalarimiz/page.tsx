...
export async function generateMetadata({ params }: { params: Promise<{ locale: Locale }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: "Mimari projelerde DecorPU poliüretan dekorasyon uygulamaları — referans çalışmalar ve kullanılan ürünler.",
...
export default async function WorksPage({ params }: { params: Promise<{ locale: Locale }> }): Promise<ReactElement> {
  ...
  return (
    <Container className="py-12 md:py-16">
      <Breadcrumbs items={[{ label: "Ana Sayfa", href: "/" }, { label: "Çalışmalarımız" }]} />
      <h1 className="mt-4 text-3xl font-semibold sm:text-4xl">Çalışmalarımız</h1>
      <p className="mt-2 max-w-2xl text-muted">Mimari projelerde DecorPU dokunuşu. Bir çalışmaya girip galeriyi ve kullanılan ürünleri görün.</p>
          ...
          <li key={w.id}>
            <Link href={`/calismalarimiz/${w.slug}`} className="group block overflow-hidden rounded-lg border border-line bg-surface transition-shadow hover:shadow-soft">
              <div className="relative aspect-[4/3] overflow-hidden bg-bg-subtle">
                {w.cover_url ? <Image src={w.cover_url} alt={w.title} fill sizes="(max-width:640px) 100vw, 33vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.05]" /> : null}
